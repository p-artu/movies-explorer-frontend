    ...
    <section className="register">
      <div className="register__container">
        ...
        <form className="register__form" onSubmit={handleSubmit} noValidate>
          ...
          <input value={values.name} onChange={handleChange} name="name" autoFocus autoComplete="off" id="name-input" type="text" className={`register__input ${errors.name ? 'register__input_type_error' : ''}`} minLength="2" maxLength="40" placeholder="латиница, кириллица, пробел или дефис" required pattern="^[a-zA-Zа-яА-ЯЁё\s\-]+$"/>
          ...
          <button className={`register__submit ${isValid ? '' : 'register__submit_disabled'}`} type={isValid ? 'submit' : 'button'}>Зарегистрироваться</button>
        </form>
